...
//--------------------------------------------------------------------------------
int16 PWM1AQ_SET, PWM2AQ_SET, PWM3AQ_SET, PWM_PRD, PWM1A_CMP, PWM2A_CMP,
		PWM3A_CMP, PWM4A_CMP, PWM5A_CMP, PWM1A_test, PWM2A_test, PWM3A_test;
Uint16 Cnt_PWM, Cnt_APP_1ms, Cnt_250us, Cnt_ms, Cnt_sec, Cnt_Dir01F, Cnt_Dir01B,
		Cnt_Dir02F, Cnt_Dir02B, Cnt_Spd1, Cnt_Spd2;
...
//--------------------------------------------------------------------------------
int16 AD_CH[30], DA_CH[5], FPGA_RD[10], FPGA_WR[10], DPRAM_RD[40], DPRAM_WR[40],
		EEPROM_RD[40], EEPROM_WR[40];
Uint16 ERR_OS[5];
...
void main(void) {
	...
	EALLOW;
	PieVectTable.EPWM1_INT = &EPWM1_isr;  //3.1
	PieVectTable.TINT0 = &RTOStimer_isr; //1.7
	EDIS;
	...
	InitCpuTimers();
	ConfigCpuTimer(&CpuTimer0, 150, 200); // 200us 150MHz CPU Freq, 1 second Period (in uSeconds)
...
void SP_PRO(void) {
		...
		if ((abs(EQep1Regs.QPOSLAT - Ro_SP01.pos_pre)) >= 10) {
			...
			if (Ro_SP01.dir == 0x11) {
				if (Ro_SP01.pos_cur > Ro_SP01.pos_pre) //锟斤拷�锟较达拷说锟斤拷锟斤拷锟剿伙拷械锟斤拷锟?
						{
					Ro_SP01.pos_delta = Ro_SP01.pos_pre + 0xFFFFFFFF
							- Ro_SP01.pos_cur;
				} else {
				...
				if (Ro_SP01.pos_cur < Ro_SP01.pos_pre) //锟斤拷�锟斤拷小说锟斤拷锟斤拷锟剿伙拷械锟斤拷锟?
						{
					Ro_SP01.pos_delta = Ro_SP01.pos_cur + 0xFFFFFFFF
							- Ro_SP01.pos_pre;
				} else {
					Ro_SP01.pos_delta = Ro_SP01.pos_cur - Ro_SP01.pos_pre;
				}
			}
			Ro_SP01.omega_m_high = ((float) Ro_SP01.pos_delta) * 37.5
					/ Cnt_Spd1; //Ro_SP01.omega_m_high * 0.99 +  * 0.01//1.25,Ro_SP01.pos_delta * (150MHz/QUPRD) / 4 / 20 / 4; 20ms,4 pulses per PRD,20 pulses per TR per Min,4 div frequency
			Cnt_Spd1 = 1;
		} else {
			Cnt_Spd1++;
			if (Cnt_Spd1 >= 10000)
		...
		if ((abs(EQep2Regs.QPOSLAT - Ro_SP02.pos_pre)) >= 10) {
			...
			if (Ro_SP02.dir == 0x11) {
				if (Ro_SP02.pos_cur > Ro_SP02.pos_pre) //锟斤拷�锟较达拷说锟斤拷锟斤拷锟剿伙拷械锟斤拷锟?
						{
					Ro_SP02.pos_delta = Ro_SP02.pos_pre + 0xFFFFFFFF
							- Ro_SP02.pos_cur;
				} else {
				...
				if (Ro_SP02.pos_cur < Ro_SP02.pos_pre) //锟斤拷�锟斤拷小说锟斤拷锟斤拷锟剿伙拷械锟斤拷锟?
						{
					Ro_SP02.pos_delta = Ro_SP02.pos_cur + 0xFFFFFFFF
							- Ro_SP02.pos_pre;
				} else {
					Ro_SP02.pos_delta = Ro_SP02.pos_cur - Ro_SP02.pos_pre;
				}
			}
			Ro_SP02.omega_m_high = ((float) Ro_SP02.pos_delta) * 37.5
					/ Cnt_Spd2; //Ro_SP01.omega_m_high * 0.99 +  * 0.01//1.25,Ro_SP01.pos_delta * (150MHz/QUPRD) / 4 / 20 / 4; 20ms,4 pulses per PRD,20 pulses per TR per Min,4 div frequency
			Cnt_Spd2 = 1;
		} else {
			Cnt_Spd2++;
			if (Cnt_Spd2 >= 10000)
	...
	AI[28] = (Ro_SP01.dir & 0x000F) + ((Ro_SP02.dir & 0x000F) << 4)
			+ ((Ro_SP01.dir & 0x000F) << 8) + ((Ro_SP02.dir & 0x000F) << 12);
...
void DPRAM_PRO_RD(void) {
	...
	CMD_OS[5] = ((*(XintfZone6 + 0x02) & 0x00FF)
			+ ((*(XintfZone6 + 0x03) << 8) & 0xFF00)) * 0.0000001;
	CMD_OS[6] = ((*(XintfZone6 + 0x04) & 0x00FF)
			+ ((*(XintfZone6 + 0x05) << 8) & 0xFF00)) * 0.0000001;
	CMD_OS[7] = ((*(XintfZone6 + 0x06) & 0x00FF)
			+ ((*(XintfZone6 + 0x07) << 8) & 0xFF00)) * 0.0000001;
	CMD_OS[8] = ((*(XintfZone6 + 0x08) & 0x00FF)
			+ ((*(XintfZone6 + 0x09) << 8) & 0xFF00)) * 0.0000001;
	CMD_OS[9] = ((*(XintfZone6 + 0x0A) & 0x00FF)
			+ ((*(XintfZone6 + 0x0B) << 8) & 0xFF00)) * 0.0000001;

	CMD_OS[10] = ((*(XintfZone6 + 0x0C) & 0x00FF)
			+ ((*(XintfZone6 + 0x0D) << 8) & 0xFF00)) * 0.1;
	CMD_OS[11] = ((*(XintfZone6 + 0x0E) & 0x00FF)
			+ ((*(XintfZone6 + 0x0F) << 8) & 0xFF00)) * 0.1;
	CMD_OS[12] = ((*(XintfZone6 + 0x10) & 0x00FF)
			+ ((*(XintfZone6 + 0x11) << 8) & 0xFF00));
	CMD_OS[13] = ((*(XintfZone6 + 0x12) & 0x00FF)
			+ ((*(XintfZone6 + 0x13) << 8) & 0xFF00));
	CMD_OS[14] = ((*(XintfZone6 + 0x14) & 0x00FF)
			+ ((*(XintfZone6 + 0x15) << 8) & 0xFF00));
	CMD_OS[15] = ((*(XintfZone6 + 0x16) & 0x00FF)
			+ ((*(XintfZone6 + 0x17) << 8) & 0xFF00));

	CMD_OS[16] = (*(XintfZone6 + 0x18) & 0x00FF);			//Motor contr Mode

	CMD_OS[17] = (*(XintfZone6 + 0x19) & 0x00FF);			//Motor contr Mode

	CMD_OS[18] = ((*(XintfZone6 + 0x1A) & 0x00FF)
			+ ((*(XintfZone6 + 0x1B) << 8) & 0xFF00)) * 0.01;
	CMD_OS[19] = ((*(XintfZone6 + 0x1C) & 0x00FF)
			+ ((*(XintfZone6 + 0x1D) << 8) & 0xFF00));
	CMD_OS[20] = ((*(XintfZone6 + 0x1E) & 0x00FF)
			+ ((*(XintfZone6 + 0x1F) << 8) & 0xFF00)) * 0.1;
	CMD_OS[21] = ((*(XintfZone6 + 0x20) & 0x00FF)
			+ ((*(XintfZone6 + 0x21) << 8) & 0xFF00)) * 0.1;

	AI[29] = ((*(XintfZone6 + 0x22) & 0x00FF)
			+ ((*(XintfZone6 + 0x23) << 8) & 0xFF00)) * 0.01;	//ambient temp
	DPRAM_RD[0] = ((*(XintfZone6 + 0x24) & 0x00FF)
			+ ((*(XintfZone6 + 0x25) << 8) & 0xFF00));
	DPRAM_RD[1] = ((*(XintfZone6 + 0x26) & 0x00FF)
			+ ((*(XintfZone6 + 0x27) << 8) & 0xFF00)) * 0.1;
	...
	CMD_OS[23] = ((*(XintfZone6 + 0x40) & 0x00FF)
			+ ((*(XintfZone6 + 0x41) << 8) & 0xFF00)) * 0.01;
	CMD_OS[24] = ((*(XintfZone6 + 0x42) & 0x00FF)
			+ ((*(XintfZone6 + 0x43) << 8) & 0xFF00)) * 0.01;
	CMD_OS[25] = ((*(XintfZone6 + 0x44) & 0x00FF)
			+ ((*(XintfZone6 + 0x45) << 8) & 0xFF00)) * 0.01;
	CMD_OS[26] = ((*(XintfZone6 + 0x46) & 0x00FF)
			+ ((*(XintfZone6 + 0x47) << 8) & 0xFF00)) * 0.01;
	CMD_OS[27] = ((*(XintfZone6 + 0x48) & 0x00FF)
			+ ((*(XintfZone6 + 0x49) << 8) & 0xFF00)) * 0.01;

	CMD_OS[28] = ((*(XintfZone6 + 0x4A) & 0x00FF)
			+ ((*(XintfZone6 + 0x4B) << 8) & 0xFF00)) * 0.01;
	CMD_OS[29] = ((*(XintfZone6 + 0x4C) & 0x00FF)
			+ ((*(XintfZone6 + 0x4D) << 8) & 0xFF00)) * 0.01;
	CMD_OS[30] = ((*(XintfZone6 + 0x4E) & 0x00FF)
			+ ((*(XintfZone6 + 0x4F) << 8) & 0xFF00)) * 0.01;
	CMD_OS[31] = ((*(XintfZone6 + 0x50) & 0x00FF)
			+ ((*(XintfZone6 + 0x51) << 8) & 0xFF00)) * 0.01;
	CMD_OS[32] = ((*(XintfZone6 + 0x52) & 0x00FF)
			+ ((*(XintfZone6 + 0x53) << 8) & 0xFF00)) * 0.01;

	CMD_OS[33] = ((*(XintfZone6 + 0x54) & 0x00FF)
			+ ((*(XintfZone6 + 0x55) << 8) & 0xFF00)) * 0.01;
	CMD_OS[34] = ((*(XintfZone6 + 0x56) & 0x00FF)
			+ ((*(XintfZone6 + 0x57) << 8) & 0xFF00)) * 0.01;
	CMD_OS[35] = ((*(XintfZone6 + 0x58) & 0x00FF)
			+ ((*(XintfZone6 + 0x59) << 8) & 0xFF00));		//status requirement
	CMD_OS[36] = ((*(XintfZone6 + 0x5A) & 0x00FF)
			+ ((*(XintfZone6 + 0x5B) << 8) & 0xFF00)) * 0.01;
	CMD_OS[37] = ((*(XintfZone6 + 0x5C) & 0x00FF)
			+ ((*(XintfZone6 + 0x5D) << 8) & 0xFF00)) * 0.01;

	CMD_OS[1] = (*(XintfZone6 + 0x1FB) & 0x00FF)
			+ ((*(XintfZone6 + 0x1FC) << 8) & 0xFF00);				//MCU status
	CMD_OS[0] = (*(XintfZone6 + 0x1FD) & 0x00FF)
			+ ((*(XintfZone6 + 0x1FE) << 8) & 0xFF00);			//MCU version
...
void DPRAM_PRO_WR(void) {
	...
	*(XintfZone6 + 0x21A) = (((int16) CTRL_OS[16] >> 8) & 0xFF);		//Ic RMS
	...
	for (i = 0; i <= 5; i++) {
		...
		*(XintfZone6 + 0x21C + (2 * i)) = (((int16) (AI[(18 + i)] * 100) >> 8)
				& 0xFF);
	}
	...
	for (i = 0; i <= 17; i++) {
		...
		*(XintfZone6 + 0x234 + (2 * i)) = (((int16) (AI[(0 + i)] * 10) >> 8)
				& 0xFF);
	}
	//error collections
	for (i = 0; i <= 4; i++) {
...
void InitVariables(void) {
	...
	Ro_SP01.dir = 1;
	Ro_SP01.omega_m = 0;
	Ro_SP01.omega_m_medium = 0;
	Ro_SP01.omega_m_high = 0;
	Ro_SP01.omega_m_low = 0;
	Ro_SP01.pos_cur = 0;
	Ro_SP01.pos_delta = 0;
	Ro_SP01.pos_pre = 0;
	Ro_SP02.dir = 2;
	Ro_SP02.omega_m = 0;
	Ro_SP02.omega_m_medium = 0;
	Ro_SP02.omega_m_high = 0;
	Ro_SP02.omega_m_low = 0;
	Ro_SP02.pos_cur = 0;
	Ro_SP02.pos_delta = 0;
	Ro_SP02.pos_pre = 0;
